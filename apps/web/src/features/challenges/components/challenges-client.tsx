...
export function ChallengesClient({ challenges, isAuthenticated }: Props) {
  const [activeTab, setActiveTab] = useState<ChallengeCategory>('frontend')
  ...
  return (
    <div>
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as ChallengeCategory)}>
        <TabsList>
        ...
        <TabsContent value="frontend">
          {frontend.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center">No challenges available.</p>
          ) : (
            <div className="flex flex-col gap-3">
              {frontend.map((challenge) => (
                <ChallengeCard
                  key={challenge.id}
                  challenge={challenge}
